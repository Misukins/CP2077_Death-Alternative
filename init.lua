...
function revivePlayer(player)
	...
	if player and IProps.canDrawDeathScreen then
		...
		elseif (useVApartment == true) then
...
function runUpdates(player)
	...
	if (useOldFeatures == true) and (useMod == true) then
...
registerForEvent("onDraw", function()
	if (showSettings) then
		...
		ImGui.Separator()
		state, pressed = ImGui.Checkbox("Use this mod", useMod)
		if pressed then 
			useMod = state
		end
		ImGui.Separator()
		...
		ImGui.Separator()
		state, pressed = ImGui.Checkbox("Teleport to V's Apartment", useVApartment)
		if pressed then 
			useVApartment = state
			useOldFeatures 	= false
			useRipperDocs 	= false
